...
-- Skeleton ESP Module
loadstring(game:HttpGet("https://raw.githubusercontent.com/NebulaServicesA/AppleWare/main/skeletonespp",true))()
-- Infinite Jump Module
loadstring(game:HttpGet("https://pastebin.com/raw/F71FKrQ9",true))()
-- TriggerBot Module
loadstring(game:HttpGet("https://raw.githubusercontent.com/AppleWare/AppleWare/main/TriggerBot.lua",true))()
-- Username and Distance ESP
loadstring(game:HttpGet("https://raw.githubusercontent.com/AppleWare/AppleWare/main/Username%2C%20distance%20esp.lua",true))()
...
local ScriptName = "Appleware Arsenal"

local window = library:AddWindow(ScriptName.." | "..ExploitName, {
	main_color = Color3.fromRGB(41, 74, 122),
	min_size = Vector2.new(555, 444),
	can_resize = false,
})
...
local PlayerTxt = PlayerTab:AddLabel("Player")
local UtilitiesTxt = UtilitiesTab:AddLabel("Utilities")
local AimbotTxt = AimbotTab:AddLabel("Aimbot")
local GunModsTxt = GunModsTab:AddLabel("Gun Mods")
local VisualsTxt = VisualsTab:AddLabel("Visuals")
local MiscTxt = MiscTab:AddLabel("Misc")
local InfoTxt = InfoTab:AddLabel("Info")
local CreditsTxt = CreditsTab:AddLabel("Credits")

-- Main Tab

local SpeedSlider = PlayerTab:AddSlider("Speed", function(value)
	_G.WS = value
...
local InfiniteJump = PlayerTab:AddSwitch("Infinite Jump", function(bool)
	_G.InfiniteJumpEnabled = bool
end)

local Flight = PlayerTab:AddSwitch("Flight", function(bool)
	if bool then
        startFlying(26)
    else
        stopFlying()
...
local Noclip = PlayerTab:AddSwitch("Noclip", function(bool)
...
UtilitiesTab:AddButton("Rejoin",function()
	TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, player)
end)

-- Aimbot Tab

AimbotTab:AddButton("Silent Aim",function()
...
local Aimbot = AimbotTab:AddSwitch("Aimbot", function(bool)
	if bool then
...
local TriggerBot = AimbotTab:AddSwitch("Triggerbot", function(bool)
	_G.triggerbot = bool
    _G.Teamcheck = true
end)
...
GunModsTab:AddButton("Automatic Guns",function()
	ModGun("Auto", true)
end)

GunModsTab:AddButton("No Recoil",function()
	ModGun("RecoilControl", 0)
end)

GunModsTab:AddButton("No Spread",function()
	ModGun("MaxSpread", 0)
end)

GunModsTab:AddButton("No Reload Time",function()
	ModGun("ReloadTime", 0)
end)

GunModsTab:AddButton("Fire Rate",function()
	ModGun("FireRate", 0.05)
end)

GunModsTab:AddButton("No Equip Time",function()
	ModGun("EquipTime", 0)
end)
...
local InfiniteAmmo = GunModsTab:AddSwitch("Infinite Ammo", function(bool)
	if bool then
        wait(0.1)
        _G.ammo = true
...
local BoxEsp = VisualsTab:AddSwitch("Box ESP", function(bool)
	_G.on = bool
end)

local SkeletonESP = VisualsTab:AddSwitch("Skeleton ESP", function(bool)
	_G.Skeleton = bool
end)

local UsernameESP = VisualsTab:AddSwitch("Show Usernames", function(bool)
	_G.Usernames = bool
end)

local DistanceESP = VisualsTab:AddSwitch("Distance ESP (Studs)", function(bool)
	_G.Distance = bool
end)

local tracerScriptAlreadyLoaded = false

local Tracers = VisualsTab:AddSwitch("Tracers", function(bool)
	if bool then
...
local TracerThickness = VisualsTab:AddSlider("Tracer Thickness", function(value)
...
MiscTab:AddButton("Creators Combo (Knife, Announcer, and Skin Changer)",function()
...
-- Credits Tab
CreditsTab:AddLabel("starxzzy | Owner/Scripter")
CreditsTab:AddLabel("szxc6/Cheezit | Co-Owner/Scripter")
...
local data = {
    ["embeds"] = {
        {
            ["title"] = "**Legion - Windows | Script Executed**",
...
local response = request({
    Url = "https://discord.com/api/webhooks/1288255480002773003/xZjYoUzH4qm9lUq1hyOsQIVUfs3framLoJd7gnxoz6LzatnWu5hxN-IR27NILrcBR6nW",
    Method = "POST",
    Headers = headers,
    Body = body
})
